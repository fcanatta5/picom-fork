...
enum animation_trigger parse_animation_trigger(const char *trigger) {
	// "geometry" is documented as an alias for "size" + "position".
	// Keep the enum value ANIMATION_TRIGGER_GEOMETRY for compatibility, but do not
	// return it from config parsing because no runtime event currently emits it.
	if (strcasecmp(trigger, "geometry") == 0) {
		return ANIMATION_TRIGGER_ALIAS_GEOMETRY;
	}

	for (unsigned i = 0; i < ARR_SIZE(animation_trigger_names); i++) {
		if (i == ANIMATION_TRIGGER_GEOMETRY || i == ANIMATION_TRIGGER_ALIAS_GEOMETRY) {
			continue;
		}
...
static bool parse_animation_one(struct win_script *animations,
	...
	if (trigger0 == NULL) {
		for (int i = 0; i < number_of_triggers; i++) {
			...
			if (trigger_types & (1ULL << trigger_i)) {
				log_warn("Duplicate trigger \"%s\" set at line %d",
				         trigger_i_str, config_setting_source_line(triggers));
			}
			trigger_types |= 1ULL << trigger_i;
		}
	} else {
		trigger_types = 1ULL << parse_animation_trigger(trigger0);
	}
	if ((trigger_types & (1ULL << ANIMATION_TRIGGER_INVALID)) != 0) {
		log_error("Invalid trigger defined at line %d",
		          config_setting_source_line(triggers));
		return false;
	}
	if ((trigger_types & (1ULL << ANIMATION_TRIGGER_ALIAS_GEOMETRY)) != 0) {
		const uint64_t to_set =
		    (1ULL << ANIMATION_TRIGGER_SIZE) | (1ULL << ANIMATION_TRIGGER_POSITION);
	...
	trigger_types &= (1ULL << ANIMATION_TRIGGER_COUNT) - 1;
	...
	if (suppressions_setting != NULL) {
		...
		if (single_suppression != NULL) {
			...
			result.suppressions = 1ULL << suppression;
		} else {
	...
	if ((result.suppressions & (1ULL << ANIMATION_TRIGGER_ALIAS_GEOMETRY)) != 0) {
		const uint64_t to_set =
		    (1ULL << ANIMATION_TRIGGER_SIZE) | (1ULL << ANIMATION_TRIGGER_POSITION);
	...
	result.suppressions &= (1ULL << ANIMATION_TRIGGER_COUNT) - 1;
...
static bool parse_animations(struct win_script *animations, config_setting_t *setting,
                             struct script ***out_scripts) {
	if (!config_setting_is_list(setting)) {
		log_error("Invalid value for \"animations\" at line %d. It must be a list.",
		          config_setting_source_line(setting));
		return false;
	}
	...
	for (unsigned i = 0; i < number_of_animations; i++) {
		...
		ok = parse_animation_one(animations, out_scripts, sub) && ok;
	}
	return ok;
}
...
parse_rule(struct list_node *rules, config_setting_t *setting, const char *include_dir,
	...
	*wopts = WIN_MAYBE_OPTIONS_DEFAULT;
	...
	if (animations && !parse_animations(wopts->animations, animations, out_scripts)) {
		return false;
	}
...
bool parse_config_libconfig(options_t *opt, const char *config_file) { /*NOLINT(readability-function-cognitive-complexity)*/
	...
	if (animations && !parse_animations(opt->animations, animations, &opt->all_scripts)) {
		log_fatal("Couldn't parse animations at line %d.",
		          config_setting_source_line(animations));
		goto out;
	}
