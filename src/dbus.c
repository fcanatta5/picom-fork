...
cdbus_process_windows(DBusConnection *conn, DBusMessage *msg, void *ud) {
	...
	if (*endptr != '\0') {
		...
		if (trigger == ANIMATION_TRIGGER_ALIAS_GEOMETRY) {
			trigger_mask = (1ULL << ANIMATION_TRIGGER_SIZE) |
			               (1ULL << ANIMATION_TRIGGER_POSITION);
		} else if (trigger < ANIMATION_TRIGGER_INVALID) {
			trigger_mask = 1ULL << trigger;
		} else {
		...
		unsigned count = 0;
		if (w != NULL) {
			for (unsigned current_trigger = 0;
			     current_trigger < ANIMATION_TRIGGER_COUNT; current_trigger++) {
				if ((trigger_mask & (1ULL << current_trigger)) == 0) {
					continue;
				}
				if (block) {
					w->animation_block[current_trigger] += 1;
				} else if (w->animation_block[current_trigger] > 0) {
					w->animation_block[current_trigger] -= 1;
				}
				count += w->animation_block[current_trigger];
			}
		}
